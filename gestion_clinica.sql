...
-- Consulta 1 Se solicita mostrar el nombre de cada paciente y el nombre del doctor con el que tienen una cita asignada.
SELECT
    (SELECT Nombre FROM Pacientes WHERE ID = c.Paciente_ID) AS Nombre_Paciente,
    (SELECT Nombre FROM Doctores WHERE ID = c.Doctor_ID) AS Nombre_Doctor
FROM Citas c;


-- Consulta 2 Se solicita una lista del Nombre y la Especialidad de todos los doctores que atendieron al paciente “Juan Pérez”
SELECT
    (SELECT Nombre FROM Doctores WHERE ID = c.Doctor_ID) AS Nombre_Doctor,
    (SELECT Nombre FROM Especialidades WHERE ID = (SELECT Especialidad_ID FROM Doctores WHERE ID = c.Doctor_ID)) AS Especialidad
FROM Citas c
WHERE c.Paciente_ID = (SELECT ID FROM Pacientes WHERE Nombre = 'Juan Pérez');


-- Consulta 3 Se solicita una lista con el monto que factura cada doctor, ordenados alfabéticamente de forma descendente, agrupados por el nombre del Doctor.
SELECT
    (SELECT Nombre FROM Doctores WHERE ID = c.Doctor_ID) AS Nombre_Doctor,
    (SELECT SUM(Monto) FROM Facturas WHERE ID = c.Factura_ID) AS Total_Facturado
FROM Citas c
ORDER BY Nombre_Doctor DESC;


-- Consulta 4 Se solicita una lista que muestre por paciente, el diagnóstico y tratamiento que recibió en su última cita. Ordenados de forma ascendente por el id del paciente.
SELECT
...
WHERE (hc.Paciente_ID, hc.Fecha_Cita) IN (
    SELECT Paciente_ID, MAX(Fecha_Cita)
    FROM Historia_Clinica
    GROUP BY Paciente_ID
)
ORDER BY p.ID ASC;

-- Consulta 5 Se solicita una lista con la cantidad de pacientes que ha atendido cada doctor.
SELECT
    Nombre,
    (SELECT COUNT(DISTINCT Paciente_ID) FROM Citas WHERE Doctor_ID = Doctores.ID) AS Cantidad_Pacientes
FROM Doctores;
